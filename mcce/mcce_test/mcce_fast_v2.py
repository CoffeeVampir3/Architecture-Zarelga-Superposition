...
import mcce_fast  # reuse naive ref, chunked fallback, _validate_inputs
...
if _TRITON_AVAILABLE:
    ...
    @triton.autotune(configs=_FWD_CONFIGS, key=["T", "V", "D", "S_MAX"])
    @triton.jit
    def _mcce_fwd_fused_kernel(
        H, E, LABELS, S, LSE, TARGET_SUM,
        T: tl.constexpr, V: tl.constexpr, D: tl.constexpr, S_MAX: tl.constexpr,
    ...
    @triton.autotune(configs=_BWD_CONFIGS, key=["T", "V", "D", "S_MAX"], reset_to_zero=["DH"])
    @triton.jit
    ...
    @triton.autotune(configs=_BWD_CONFIGS, key=["T", "V", "D", "S_MAX"], reset_to_zero=["DE"])
    @triton.jit
