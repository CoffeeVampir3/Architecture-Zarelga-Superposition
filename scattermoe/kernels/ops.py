...
def rowwise_sum_product(A, B, dtype=None):
    assert A.dim() == 3
    assert B.dim() == 2
    ...
    T, F, H = A.shape
    output = torch.empty((T, F), device=A.device, dtype=dtype or A.dtype)
    rowwise_sum_product_compileable(output, A, B)
    return output


@torch.library.custom_op("scattermoe::rowwise_sum_product", mutates_args={"output"})
def rowwise_sum_product_compileable(
        output: torch.Tensor,
        A: torch.Tensor,
        B: torch.Tensor) -> None:
    H = A.size(2)
    block_h = triton.next_power_of_2(H)
    _rowwise_sum_product[(A.size(0), A.size(1))](
        A,
        A.stride(0),
        A.stride(1),
        A.stride(2),
...
@rowwise_sum_product_compileable.register_fake
def _(output: torch.Tensor, A: torch.Tensor, B: torch.Tensor) -> None:
    return None


@triton.jit
def _rowwise_sum_product(
    A_ptr,
    stride_at: tl.constexpr,
    stride_af: tl.constexpr,
    ...
    stride_of: tl.constexpr,
    H: tl.constexpr,
    BLOCK_H: tl.constexpr,
):
    t_idx = tl.program_id(axis=0)
    f_idx = tl.program_id(axis=1)
    h_offsets = tl.arange(0, BLOCK_H)
    h_mask = h_offsets < H

    a = tl.load(
        A_ptr + t_idx * stride_at + f_idx * stride_af + h_offsets * stride_ah,
        mask=h_mask,
        other=0.0,
    )
    b = tl.load(
        B_ptr + t_idx * stride_bt + h_offsets * stride_bh,
        mask=h_mask,
        other=0.0,
    )
    acc = tl.sum(a.to(tl.float32) * b.to(tl.float32), axis=0)
    tl.store(output_ptr + t_idx * stride_ot + f_idx * stride_of, acc)
