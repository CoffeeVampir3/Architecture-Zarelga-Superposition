...
def compileable_bincount(x: torch.Tensor, minlength: int) -> torch.Tensor:
        # Avoid torch.bincount on CUDA: it calls max().item() internally to size
        # the output, which forces a device->host sync and breaks CUDA graph
        # capture (mode="max-autotune"). minlength is the known upper bound on
        # the index range here (num_experts), so scatter_add is equivalent.
        out = torch.zeros(minlength, dtype=torch.long, device=x.device)
        ones = torch.ones_like(x, dtype=torch.long)
        out.scatter_add_(0, x.long(), ones)
        return out
...
class ParallelLinear(torch.autograd.Function):
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        if gates is not None:
            # Autocast can make grad_out BF16 while the saved expert output is FP32.
            d_gates = kernels.ops.rowwise_sum_product(output_expanded, grad_out, dtype=gates.dtype)
